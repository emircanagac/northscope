...
func (w *Watcher) Run(ctx context.Context) error {
	log.Printf("NorthScope Kubernetes watcher starting")
	...
	log.Printf("NorthScope Kubernetes informer caches synced")
...
func (w *Watcher) rebuildAndPublish() {
	...
	if firstSnapshot {
		log.Printf(
			"NorthScope ready: topology snapshot v%d generated with %d nodes and %d edges",
			snapshot.Version,
			len(snapshot.Nodes),
			len(snapshot.Edges),
		)
...
func (w *Watcher) buildSnapshot() (models.TopologySnapshot, error) {
	...
	externalResources := listOptionalExternalResources(context.Background(), w.dynamicClient)
