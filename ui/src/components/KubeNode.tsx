      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div
            title={title}
            className={cx(
              'mt-1.5 line-clamp-2 break-words font-bold leading-tight text-slate-950',
              titleSize,
            )}
          >
            {title}
          </div>
          {data.namespace ? <div className="mt-1 max-w-full truncate text-xs font-medium text-slate-500">{data.namespace}</div> : null}
        </div>
      </div>
      {summaryLine ? (
        <div
          title={summaryLine}
          className={cx(
            'min-h-[24px] max-w-full overflow-hidden truncate whitespace-nowrap rounded-md bg-white/75 px-2 py-1 font-medium leading-4 text-slate-600',
            'text-[11px]',
